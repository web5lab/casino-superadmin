...
import { depositesApi } from '../store/global.Action';

export default function TransactionsPage() {
  const [isFilterOpen, setIsFilterOpen] = useState(false);
  const [selectedTx, setSelectedTx] = useState(null);
  const [activeFilters, setActiveFilters] = useState({});
  const deposites = useSelector(state => state.global.deposites);
  ...
  useEffect(() => {
    dispatch(depositesApi({ token: token, casinoId: casinoId }))
  }, [])
  ...
  const filteredTransactions = deposites.filter((tx) => {
